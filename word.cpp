...
Word::Word(std::vector<Generator> gen_vec, CompMat3 matrix,
           IsomClass iso_class, comp_d trace, int order) : 
    m_gen_vec(gen_vec)
  , m_matrix(matrix)
...
std::string Word::str_isom_class(bool ell_order) const
{
    switch (m_iso_class)
    {
      case IsomClass::Identity:   return "Identity";
      case IsomClass::Elliptic:   return ell_order ? std::to_string(m_order) : "Elliptic";
      case IsomClass::Reflection: return "Reflection";
      case IsomClass::Parabolic:  return "Parabolic";
...
Word Word::invert() const
{
  ...
  Word new_word(new_vector, new_matrix, new_IsoClass, new_trace, new_order);
...
Word conjugate(const Word& base_word, const Word& conj_word)
{
  ...
  Word new_word(new_vec, new_mat, i_class, trace, order);
